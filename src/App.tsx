...
function App() {
  ...
  const addButtonHandler = async () => {
    try {
      const inputs = {
        fname: (fname.current as unknown as HTMLInputElement).value, 
        lname: (lname.current as unknown as HTMLInputElement).value
      };
			...
			<div className="flex flex-col flex-1 overflow-y-scroll">
				{
					persons.map((item) => {
						return (
							<div key={item["personid"]} className="py-2 px-4 text-teal-700 hover:bg-sky-100 hover:cursor-pointer hover:text-teal-950 text-sm border-b-[1px]">
